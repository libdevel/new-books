...
//$analyticspath = "SUNY%20Broome%20Community%20College%2001SUNY_BCC/Reports/New%20Books/new-print-titles-for-rss-v3";

$analyticspath = "SUNY%20Broome%20Community%20College%2001SUNY_BCC/Reports/New%20Books/display-print-titles-for-rss-by-barcode";
...
<rss version="2.0">
<channel>
	<title>SUNY Broome Library New Print Books on Display</title>
	<link>http://www.sunybroome.edu/library/</link>
	<description>SUNY Broome Library Book Feed : New Print Books on Display</description>
<image>
...
<?php
	foreach ($xml->QueryResult->ResultXml->rowset->Row as $row){		
		...
		$title_raw = (string) trim($row->Column6, " /");
		$title = preg_replace('/&(?![#]?[a-z0-9]+;)/i', "&amp;$1", $title_raw);
		$pubdate_raw = (string) $row->Column4;
		preg_match ( '/\d{4}/', $pubdate_raw, $publication_date);
		$lc_code = (string) $row->Column7;
	...
	if (strlen($isbn_array[0]) > 0) {
		...
		$image_size_s = getimagesize($image_s);
			if ($image_size_s[0] > 1 and $image_size_s[1] > 1) {
				$image_src = '<img src="' . $image_s . '" class="book_cover" height="150" '. $alt . ' />';
